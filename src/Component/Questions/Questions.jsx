  ...
  return (
    <>
     <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-blue-300 via-blue-400 to-blue-500 py-20 px-4">
      <div style={{backgroundColor:colors.white}} className="w-full md:w-2/5 rounded-3xl  bg-opacity-90 p-8 shadow-xl">
          ...
          {faqs.map((faq, index) => (
            <div key={index}>
              <dt className="flex items-center justify-between cursor-pointer py-4" onClick={() => toggleFAQ(index)}>
                ...
                {openIndex === index ? (
                  <Minus style={{color:colors.primary}} className="w-5 h-5 " />
                ) : (
                  <Plus style={{color:colors.primary}} className="w-5 h-5 " />
                )}
              </dt>
              {openIndex === index && (
